	...
	$obj = new project(array('id'=>$_REQUEST['id']));
	?>
	<!----profession----->
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="newForm">
						<style type="text/css">
							.input-group-text-project {
								width: 150px;
							}
						</style>
						<div class="input-group mb-3">
							<div class="input-group-prepend">
								<span class="input-group-text input-group-text-project" id="basic-addon1">Название</span>
							</div>
							<input type="text" class="form-control" aria-describedby="basic-addon1"  name="name" id="project_name" value="<?php echo $obj->name;?>">
						</div>
						<div class="input-group mb-3" style="border: 1px solid #ced4da; border-radius: .25rem;">
							<div class="input-group-prepend">
								<span class="input-group-text input-group-text-project" id="basic-addon2">Администраторы</span>
							</div>
...
main::printr($user);
									...
									foreach ($obj->admin as $adminID) 
									{

										if($user->id == $adminID['user'])
										{
											$check = true;
										};
									};
									...
									if($check)
									{
										echo '<option value="'.$user->id.'" selected = "selected">"'.$user->surname.' '.$user->name.' '.$user->middlename.'"</option>';
									}
									else
									{
										echo '<option value="'.$user->id.'">"'.$user->surname.' '.$user->name.' '.$user->middlename.'"</option>';
									};
								};
								?>
							</select>
