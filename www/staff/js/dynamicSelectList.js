(function($){
	jQuery.fn.dynamicSelectList = function(){
							...
							success: function (data) {
								if (data.length>=1)
								{
									...
									$.each(data,function(i,item){
										...
										tr = tr +'<td><button type="button" class="btn btn-outline-secondary castom-menu-button editObjBtn" data-toggle="tooltip" data-placement="top" title="" data-original-title="Изменить пользователя" data-obj="' + _classData + '" data-id="' + item.id + '"><i class="material-icons">edit</i></button>&nbsp;<button type="button" class="btn btn-outline-secondary castom-menu-button deleteElementBtn" data-id="' + item.id + '" data-obj="' + _classData + '" data-toggle="tooltip" data-placement="top" title="" data-original-title="Удалить пользователя"><i class="material-icons">delete_forever</i></button></td><tr>';
										$('.tbds-list').append(tr);
									});
